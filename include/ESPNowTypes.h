 ...
 * Permite recuperação após reinicio ou queda de WiFi
 */
 ...
 * Sincronizado via ESP-NOW e guardado em NVS
 * Permite manter estados mesmo sem WiFi
 */
...
// ===== 🎯 CACHE DE ESTADOS DE SLAVES (MASTER) =====
/**
 * @brief Estado cacheado de un relé de slave (guardado en NVS del Master)
 * Permite recuperación rápida después de reinicio
 */
struct CachedSlaveRelayState {
    uint8_t slaveMac[6];        // MAC address del slave
    uint8_t relayNumber;         // Número del relé (0-7)
    uint8_t state;               // 0=OFF, 1=ON
    ...
    uint8_t isStale;             // 1=potencialmente desatualizado, 0=atualizado
    uint8_t padding[3];          // Padding para alinhamento
} __attribute__((packed));

/**
 * @brief Cache de estados de relés de TODOS os slaves
 * Guardado em NVS do Master para recuperação rápida
 */
struct SlaveRelayStatesCache {
    uint32_t timestamp;              // Timestamp da última atualização
    uint8_t version;                 // Versão do formato (1 = inicial)
    uint8_t numSlaves;               // Número de slaves no cache
    uint8_t padding[2];              // Padding para alinhamento
    CachedSlaveRelayState states[32]; // Máximo 32 estados (4 slaves x 8 relés)
    uint8_t checksum;                // Checksum para validação
} __attribute__((packed));
